...
void Shader::setUniformVariable(const std::string& name, Vector4f& value)
{
...
{
	//transpose is GL_TRUE if matrix are supplied in row major order
	glUniformMatrix4fv(glGetUniformLocation(programID, name.c_str()), 1, GL_TRUE, &value.m[0][0]);
}
